    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    GlobeView(
                        ...
                        userCountryCode: viewModel.userCountryCode
                    )
                    .frame(height: 320)
            ...
            .task {
                ...
                // Detect user's country for globe centering
                await viewModel.detectUserCountry(from: locationService.currentLocation)
            }
            .refreshable {
                _ = try? await supabaseService.getUserProfile()
                await supabaseService.loadUserStatistics()
